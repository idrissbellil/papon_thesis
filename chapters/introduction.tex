...
%The human brain is able to process a bewilderingly large amount of visual data with ease. It possesses an innate understanding of coherent motion, allowing it to keep track of and intuitively predict object trajectories. Similarly, it can easily transform a bewilderingly messy world into an ordered structure of meaningful parts and distinct objects. In this work we argue that these two concepts are inexorably linked, indeed that visual tracking plays an essential role in creating the objects we observe. We propose that without the ability to track motions in a coherent way, the notion of distinct objects is, ultimately, a meaningless one.

\chapter{Introduction}
\lettrine[lines=3, loversize=0.3]{\textcolor{DarkBlue}T}{he adult human brain} is able to process a bewilderingly large amount of visual data with ease. Nevertheless, at some point before the eyes first open, the brain is a blank slate, upon which the concepts of color, motion, objects, and so forth must be inscribed. Through observation, we learn to track objects visually through an innate understanding of coherent motion which allows us to intuitively predict object trajectories. In a related way, we also learn to break our world into meaningful parts, through both interaction and observation. In this work we argue that the two concepts are inexorably linked, that visual tracking creates the objects we see around us. We propose that without motion, and the ability to track motions in a coherent way, the notion of distinct objects is a meaningless one.
...
%Recent research into semantic labeling of video, known as \gls{vos}, has focused on extending 2D techniques into the temporal dimension. This can be accomplished by clustering into segments which are coherent in time as well as space - essentially one stacks the frames, and performs a volumetric segmentation. 
\todo[inline]{Talk about problems with volumetric segmentation techniques - particularly due to occlusions}
...
\todo[inline]{Should we add some lines about applications, or is it obvious?}
